...
export const NewMessage = () => {
  ...
  const teams = useAppSelector((state: RootState) => state.messages).teamsData
    .payload;
  const groups = useAppSelector((state: RootState) => state.messages).groups
    .payload;
  const queryGroups = useAppSelector((state: RootState) => state.messages)
    .queryGroups.payload;
  const canAccessGroups = useAppSelector((state: RootState) => state.messages)
    .verifyGroup.payload;
  ...
  const [pageSelection, setPageSelection] = React.useState(
    CurrentPageSelection.CardCreation
  );
  ...
  const [showMsgDraftingSpinner, setShowMsgDraftingSpinner] =
  ...
  const [teamsSelectedOptions, setTeamsSelectedOptions] = React.useState<
    ITeamTemplate[]
  >([]);
  const [rostersSelectedOptions, setRostersSelectedOptions] = React.useState<
    ITeamTemplate[]
  >([]);
  const [searchSelectedOptions, setSearchSelectedOptions] = React.useState<
    ITeamTemplate[]
  >([]);
  ...
  const [scheduledSendValidation, setscheduledSendValidation] =
    React.useState(true);
  const [scheduledSendTimeValidation, setscheduledSendTimeValidation] =
    React.useState(false);
  ...
  React.useEffect(() => {
    ...
    if (teams && teams.length > 0) {
      const teamsSelected = teams.filter((c) =>
        messageState.teams.some((s) => s === c.id)
      );
      setTeamsSelectedOptions(teamsSelected || []);
      const roastersSelected = teams.filter((c) =>
        messageState.rosters.some((s) => s === c.id)
      );
      setRostersSelectedOptions(roastersSelected || []);
    }
    if (groups && groups.length > 0) {
      const groupsSelected = groups.filter((c) =>
        messageState.groups.some((s) => s === c.id)
      );
    ...
    groups,
    messageState.teams,
    messageState.rosters,
    messageState.allUsers,
    messageState.groups,
  ]);
  ...
  const initializeTimePicker = () => {
    setScheduledDatePicker(
      new Date(new Date().setMinutes(new Date().getMinutes() + 30))
    );
    setScheduledTimePicker(
      new Date(new Date().setMinutes(new Date().getMinutes() + 30))
    );
  };
  ...
  const handleScheduleSendTime = (
    ...
    if (selectedTime) {
      ...
      setMessageState({
        ...messageState,
        scheduledDate: selectedTime.toISOString(),
      });
    }
  };
  ...
  const onSave = () => {
    ...
    if (selectedRadioButton === AudienceSelection.Teams) {
      finalSelectedTeams = [
        ...teams
          .filter((t1) => teamsSelectedOptions.some((sp) => sp.id === t1.id))
          .map((t2) => t2.id),
      ];
    }
    if (selectedRadioButton === AudienceSelection.Rosters) {
      finalSelectedRosters = [
        ...teams
          .filter((t1) => rostersSelectedOptions.some((sp) => sp.id === t1.id))
          .map((t2) => t2.id),
      ];
    }
  ...
  const onBtnLinkChanged = (event: any) => {
    if (
      ...
      setBtnLinkErrorMessage(
        `${event.target.value} is invalid. ${t("enterValidURL")}`
      );
    }
  ...
  const onTeamsSelect: ComboboxProps["onOptionSelect"] = (event, data) => {
    if (data.selectedOptions.length <= MAX_SELECTED_TEAMS_NUM) {
      setTeamsSelectedOptions(
        teams.filter((t1) => data.selectedOptions.some((t2) => t2 === t1.id))
      );
    }
  };
  ...
  const onSearchSelect: ComboboxProps["onOptionSelect"] = (
    event,
    data: any
  ) => {
    if (
      data.optionText &&
      !searchSelectedOptions.find((x) => x.id === data.optionValue)
    ) {
      setSearchSelectedOptions([
        ...searchSelectedOptions,
        { id: data.optionValue, name: data.optionText },
      ]);
    }
  };
  ...
  const onTeamsTagClick = (option: ITeamTemplate, index: number) => {
    ...
    setTeamsSelectedOptions(
      teamsSelectedOptions.filter((o) => o.id !== option.id)
    );

    // focus previous or next option, defaulting to focusing back to the combo input
    const indexToFocus = index === 0 ? 1 : index - 1;
    const optionToFocus = teamsSelectedListRef.current?.querySelector(
      `#${teamsComboId}-remove-${indexToFocus}`
    );
    if (optionToFocus) {
  ...
  const onRostersTagClick = (option: ITeamTemplate, index: number) => {
    // remove selected option
    setRostersSelectedOptions(
      rostersSelectedOptions.filter((o) => o.id !== option.id)
    );

    // focus previous or next option, defaulting to focusing back to the combo input
    const indexToFocus = index === 0 ? 1 : index - 1;
    const optionToFocus = rostersSelectedListRef.current?.querySelector(
      `#${rostersComboId}-remove-${indexToFocus}`
    );
    if (optionToFocus) {
      (optionToFocus as HTMLButtonElement).focus();
    } else {
  ...
  const onSearchTagClick = (option: ITeamTemplate, index: number) => {
    // remove selected option
    setSearchSelectedOptions(
      searchSelectedOptions.filter((o) => o.id !== option.id)
    );

    // focus previous or next option, defaulting to focusing back to the combo input
    const indexToFocus = index === 0 ? 1 : index - 1;
    const optionToFocus = searchSelectedListRef.current?.querySelector(
      `#${searchComboId}-remove-${indexToFocus}`
    );
    if (optionToFocus) {
      (optionToFocus as HTMLButtonElement).focus();
    } else {
  ...
  const teamsLabelledBy =
    teamsSelectedOptions.length > 0
      ? `${teamsComboId} ${teamsSelectedListId}`
      : teamsComboId;
  const rostersLabelledBy =
    rostersSelectedOptions.length > 0
      ? `${rostersComboId} ${rostersSelectedListId}`
      : rostersComboId;
  const searchLabelledBy =
    searchSelectedOptions.length > 0
      ? `${searchComboId} ${searchSelectedListId}`
      : searchComboId;
  ...
  const audienceSelectionChange = (ev: any, data: RadioGroupOnChangeData) => {
    ...
    AudienceSelection[input] === AudienceSelection.AllUsers
      ? setAllUserAria("alert")
      : setAllUserAria("none");
    AudienceSelection[input] === AudienceSelection.Groups
      ? setGroupsAria("alert")
      : setGroupsAria("none");
  };
          ...
          <div className="adaptive-task-grid new-messages">
            <div className="form-area">
              ...
              >
                  ...
                  <div className={cmbStyles.root}>
                    <Label id={teamsComboId}>{t("pickTeams")}</Label>
                    {
                      ...
                      teamsSelectedOptions.length ? (
                        <ul
                          id={teamsSelectedListId}
                          className={cmbStyles.tagsList}
                          ref={teamsSelectedListRef}
                        >
                ...
                <Radio
                  id="radio2"
                  value={AudienceSelection.Rosters}
                  ...
                  <div className={cmbStyles.root}>
                    <Label id={rostersComboId}>{t("pickTeams")}</Label>
                    {
                      ...
                      rostersSelectedOptions.length ? (
                        <ul
                          id={rostersSelectedListId}
                          className={cmbStyles.tagsList}
                          ref={rostersSelectedListRef}
                        >
                          {/* The "Remove" span is used for naming the buttons without affecting the Combobox name */}
                    ...
                    <Combobox
                      multiselect={true}
                      selectedOptions={rostersSelectedOptions.map(
                        (op) => op.id
                      )}
                      appearance="filled-darker"
                      size="large"
                ...
                <Radio
                  id="radio3"
                  value={AudienceSelection.AllUsers}
                ...
                <Radio
                  id="radio4"
                  value={AudienceSelection.Groups}
                      ...
                      <>
                        <Label id={searchComboId}>{t("pickGroups")}</Label>
                        {
                          ...
                          searchSelectedOptions.length ? (
                            <ul
                              id={searchSelectedListId}
                              className={cmbStyles.tagsList}
                              ref={searchSelectedListRef}
                            >
                              {/* The "Remove" span is used for naming the buttons without affecting the Combobox name */}
